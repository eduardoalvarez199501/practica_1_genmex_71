<div align="center">

# 👋 ¡Hola! Soy Eduardo Alvarez 

### 💻 Full Stack Developer | ☕ Java | 🌐 Web | ☁️ Cloud | 🚀 Technology Enthusiast

<p>
  <a href="https://www.linkedin.com/in/eduardo-alvarez-a4507a152/">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://github.com/eduardoalvarez199501">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <a href="mailto:eduardoalvarez199501@gmail.com">
    <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=26&pause=1000&color=36BCF7&center=true&vCenter=true&width=700&lines=Welcome+to+my+GitHub!;Software+Developer;Always+Learning+New+Technologies;Building+Solutions+with+Code"/>

</div>

---

# 🚀 About Me

```java
public class Developer {

    String name = "Eduardo Alvarez";
    String role = "Software Developer";
    String location = "Mexico";
    String[] languages = {
        "Java",
        "JavaScript",
        "TypeScript",
        "SQL"
    };

    String[] technologies = {
        "Spring Boot",
        "Node.js",
        "React",
        "Angular",
        "Docker",
        "Git"
    };

    boolean coffee = true;

    public String lifeGoal(){
        return "Hacer posible, lo que el mundo considera imposible !";
    }
}
```

---

# 🛠 Tech Stack

## Backend

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![Spring](https://img.shields.io/badge/Spring-6DB33F?style=for-the-badge&logo=spring&logoColor=white)

## Frontend

![Angular](https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular)
![JavaScript](https://img.shields.io/badge/JavaScript-black?style=for-the-badge&logo=javascript)

## Databases

![MySQL](https://img.shields.io/badge/MySQL-blue?style=for-the-badge&logo=mysql)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-blue?style=for-the-badge&logo=postgresql)
![MongoDB](https://img.shields.io/badge/MongoDB-green?style=for-the-badge&logo=mongodb)

## Cloud & DevOps
![Docker](https://img.shields.io/badge/Docker-blue?style=for-the-badge&logo=docker)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=for-the-badge&logo=kubernetes)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws)

---

# 📈 GitHub Stats

<div align="center">

![](https://github-readme-stats.vercel.app/api?username=TU_USUARIO&show_icons=true&theme=tokyonight&hide_border=true)

![](https://github-readme-streak-stats.herokuapp.com/?user=TU_USUARIO&theme=tokyonight&hide_border=true)

![](https://github-readme-stats.vercel.app/api/top-langs/?username=TU_USUARIO&layout=compact&theme=tokyonight&hide_border=true)

</div>

---

# 🏆 GitHub Trophies

<div align="center">

![](https://github-profile-trophy.vercel.app/?username=TU_USUARIO&theme=tokyonight&no-frame=true&margin-w=15)

</div>

---

# 📊 Contribution Graph

![](https://github-readme-activity-graph.vercel.app/graph?username=TU_USUARIO&theme=tokyo-night)

---

# 🌱 Currently Learning

- ☁️ Cloud Computing
- ⚙️ Microservices
- 🤖 Artificial Intelligence
- 🐳 Kubernetes
- 🔐 Cybersecurity
- 📈 Software Architecture

---

# 💼 Featured Projects

| Project | Description | Tech |
|----------|-------------|------|
| 🚀 Project One | Short description | Java • Spring |
| 📱 Project Two | Short description | React |
| ☁️ Project Three | Short description | AWS |
| 🤖 Project Four | Short description | AI |

---

# 🎯 Goals

- ✅ Keep learning every day
- ✅ Build scalable applications
- ✅ Contribute to Open Source
- ✅ Share knowledge with the community

---

# 💬 Favorite Quote

> *"First, solve the problem. Then, write the code."* – John Johnson

---

# 📫 Contact

📧 Email: **tucorreo@correo.com**

💼 LinkedIn:
https://linkedin.com/in/tuusuario

🌐 Portfolio:
https://tuportfolio.com

---

<div align="center">

### ⭐ Thanks for visiting my profile!

![](https://komarev.com/ghpvc/?username=TU_USUARIO&style=for-the-badge&color=blue)

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00C6FF,100:0072FF&height=120&section=footer"/>

</div>
